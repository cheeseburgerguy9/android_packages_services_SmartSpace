...
public class BcSmartspaceCardCombination extends BcSmartspaceCardSecondary {
  ...
  public BcSmartspaceCardCombination(Context context) { super(context); }
  public final boolean
  fillSubCard(ConstraintLayout constraintLayout, SmartspaceTarget target,
              SmartspaceAction action,
              BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier,
              BcSmartspaceCardLoggingInfo loggingInfo) {
    boolean z;
    TextView textView =
        (TextView)constraintLayout.findViewById(R.id.sub_card_text);
    ImageView imageView =
        (ImageView)constraintLayout.findViewById(R.id.sub_card_icon);
    if (textView == null) {
    ...
    constraintLayout.setContentDescription(z2 ? action.getContentDescription()
                                              : null);
    if (z2) {
  ...
  public boolean setSmartspaceActions(
    ...
    boolean z = constraintLayout != null &&
                fillSubCard(constraintLayout, target, action, eventNotifier,
                            loggingInfo);
    boolean z2 = actionChips.size() > 1 && actionChips.get(1) != null;
    boolean fillSubCard = z2 ? fillSubCard(mSecondSubCard, target,
