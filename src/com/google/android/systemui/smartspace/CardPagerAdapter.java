...
public final class CardPagerAdapter
  ...
  private final SparseArray<BaseTemplateCard> recycledCards =
      new SparseArray<>();
  private final SparseArray<BcSmartspaceCard> recycledLegacyCards =
      new SparseArray<>();
  private final SparseArray<BcSmartspaceRemoteViewsCard>
      recycledRemoteViewsCards = new SparseArray<>();
  ...
  public CardPagerAdapter(BcSmartspaceView root,
                          BcSmartspaceConfigPlugin configProvider) {
    this.root = root;
    this.configProvider = configProvider;
    int color = GraphicsUtils.getAttrColor(root.getContext(),
  ...
  public static class Companion {
    ...
    useRecycledViewForAction(SmartspaceAction newAction,
                             SmartspaceAction recycledAction) {
      Map map =
    ...
    public static boolean
    useRecycledViewForActionsList(List<SmartspaceAction> newActions,
                                  List<SmartspaceAction> recycledActions) {
      Map map =
          BcSmartspaceTemplateDataUtils.TEMPLATE_TYPE_TO_SECONDARY_CARD_RES;
      ...
      return IntStream.range(0, newActions.size())
          .allMatch(i
                    -> useRecycledViewForAction(newActions.get(i),
    ...
    public final boolean useRecycledViewForNewTarget(
      ...
      if (!newTarget.getSmartspaceTargetId().equals(
              recycledTarget.getSmartspaceTargetId())) {
        return false;
      }
      if (!useRecycledViewForAction(newTarget.getHeaderAction(),
                                    recycledTarget.getHeaderAction())) {
        return false;
      }
      if (!useRecycledViewForAction(newTarget.getBaseAction(),
                                    recycledTarget.getBaseAction())) {
        return false;
      }
      if (!useRecycledViewForActionsList(newTarget.getActionChips(),
                                         recycledTarget.getActionChips())) {
        return false;
      }
      if (!useRecycledViewForActionsList(newTarget.getIconGrid(),
                                         recycledTarget.getIconGrid())) {
        return false;
      }
  ...
  public final class ViewHolder {
    ...
    public ViewHolder(int position, BcSmartspaceCard legacyCard,
                      SmartspaceTarget target, BaseTemplateCard card,
    ...
    public void setTarget(SmartspaceTarget target) { this.target = target; }
  }
      ...
      targets.add(
          new SmartspaceTarget
              .Builder("date_card_794317_92634",
                       new ComponentName(root.getContext(),
                                         (Class<?>)CardPagerAdapter.class),
    ...
    if (holder.legacyCard != null) {
      ...
      if (smartspaceTarget != null && enableCardRecycling.get()) {
        recycledLegacyCards.put(
            BcSmartSpaceUtil.getFeatureType(smartspaceTarget),
            holder.legacyCard);
      }
  ...
  @Override
  public final BcSmartspaceRemoteViewsCard
  getRemoteViewsCardAtPosition(int position) {
    ViewHolder holder = viewHolders.get(position);
    if (holder != null) {
      return holder.remoteViewsCard;
    }
  ...
  public final SmartspaceTarget getTargetAtPosition(int position) {
    if (smartspaceTargets.isEmpty() || position < 0 ||
        position >= smartspaceTargets.size()) {
      return null;
    }
  ...
  public final Object instantiateItem(ViewGroup container, int position) {
    ...
    if (target.getRemoteViews() != null) {
      Log.i("SsCardPagerAdapter", "[rmv] Use RemoteViews for the feature: " +
                                      target.getFeatureType());
      BcSmartspaceRemoteViewsCard remoteViewsCard =
      ...
      if (remoteViewsCard == null) {
        remoteViewsCard =
            new BcSmartspaceRemoteViewsCard(container.getContext());
      ...
      boolean containsValidTemplateType =
          BcSmartspaceCardLoggerUtil.containsValidTemplateType(
      ...
      if (containsValidTemplateType) {
        ...
        BaseTemplateCard templateCard =
            enableCardRecycling.get()
                ? (BaseTemplateCard)recycledCards.removeReturnOld(
                      target.getFeatureType())
                : null;
        ...
        if (legacyCard == null || (enableReducedCardRecycling.get() &&
          ...
          if (layoutRes == 0) {
            Log.w("SsCardPagerAdapter",
                  "No legacy card can be created for feature type: " +
                      featureType);
          } else {
  ...
  public final boolean isViewFromObject(View view, Object object) {
    ViewHolder holder = (ViewHolder)object;
    return view == holder.legacyCard || view == holder.card ||
        view == holder.remoteViewsCard;
  }
  ...
  public final void onBindViewHolder(ViewHolder holder) {
    ...
    if (target.getRemoteViews() != null) {
      if (holder.remoteViewsCard == null) {
        Log.w("SsCardPagerAdapter",
              "[rmv] No RemoteViews card view can be binded");
        return;
      }
      Log.d("SsCardPagerAdapter", "[rmv] Refreshing RemoteViews card");
      holder.remoteViewsCard.bindData(
          target, dataProvider != null ? dataProvider.getEventNotifier() : null,
    ...
    BcSmartspaceCardLoggerUtil
        .tryForcePrimaryFeatureTypeOrUpdateLogInfoFromTemplateData(
  ...
  public void setDozeAmount(float dozeAmount) {
    ...
    TransitionType newTransition =
        previousDozeAmount > dozeAmount   ? TransitionType.TO_LOCKSCREEN
  ...
  @Override
  public final void
  setTimeChangedDelegate(BcSmartspaceDataPlugin.TimeChangedDelegate delegate) {
  ...
  public final void updateCurrentTextColor() {
    currentTextColor =
        ColorUtils.blendARGB(primaryTextColor, dozeColor, dozeAmount);
    for (int i = 0; i < viewHolders.size(); i++) {
      ViewHolder holder = viewHolders.valueAt(i);
      if (holder != null) {
        if (holder.legacyCard != null) {
  ...
  public void updateTargetVisibility() {
    ...
    boolean shouldUpdateLockscreen =
        ...
        (dozeAmount == 0f || (1f - dozeAmount >= 0.36f &&
