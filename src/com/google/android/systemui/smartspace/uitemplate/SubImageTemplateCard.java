...
public class SubImageTemplateCard extends BcSmartspaceCardSecondary {
  ...
  public SubImageTemplateCard(Context context) { this(context, null); }

  public SubImageTemplateCard(Context context, AttributeSet attrs) {
    super(context, attrs);
    mIconDrawableCache = new HashMap<>();
    mHandler = new Handler();
    mImageHeight = getResources().getDimensionPixelOffset(
  ...
  public final boolean setSmartspaceActions(
    ...
    SubImageTemplateData templateData =
        (SubImageTemplateData)target.getTemplateData();
    if (!BcSmartspaceCardLoggerUtil.containsValidTemplateType(templateData) ||
      ...
      Log.w(TAG, "SubImageTemplateData is null or has no SubImage or invalid " +
                 "template type");
      return false;
    }
  ...
  private void createAndStartAnimation(TreeMap<Integer, Drawable> drawables,
                                       int duration,
                                       WeakReference<ImageView> imageViewRef) {
    List<Drawable> validDrawables = drawables.values()
                                        .stream()
  ...
  private static final class LoadUriTask
    ...
    protected DrawableWrapper doInBackground(DrawableWrapper... wrappers) {
      ...
      try (InputStream inputStream =
               wrapper.mContentResolver.openInputStream(wrapper.mUri)) {
        ...
        wrapper.mDrawable =
            ImageDecoder.decodeDrawable(source, (decoder, info, src) -> {
              ...
              float ratio = height != 0
                                ...
                                : 0.0f;
              decoder.setTargetSize((int)(wrapper.mHeightInPx * ratio),
                                    wrapper.mHeightInPx);
            });
      } catch (Exception e) {
