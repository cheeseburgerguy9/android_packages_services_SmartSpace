...
public class CarouselTemplateCard extends BcSmartspaceCardSecondary {
  public CarouselTemplateCard(Context context) { super(context); }
  ...
  public final void onFinishInflate() {
    ...
    for (int i = 0; i < 4; i++) {
      ConstraintLayout column = (ConstraintLayout)ViewGroup.inflate(
          getContext(), R.layout.smartspace_carousel_column_template_card,
          null);
  ...
  public final boolean setSmartspaceActions(
    ...
    CarouselTemplateData templateData =
        (CarouselTemplateData)target.getTemplateData();
    if (!BcSmartspaceCardLoggerUtil.containsValidTemplateType(templateData) ||
        templateData.getCarouselItems() == null) {
      Log.w("CarouselTemplateCard", "CarouselTemplateData is null or has no " +
                                    "CarouselItem or invalid template type");
      return false;
    }
    List<CarouselTemplateData.CarouselItem> carouselItems =
        templateData.getCarouselItems();
    ...
    if (validItems < 4) {
      Log.w("CarouselTemplateCard",
            "Hiding " + (4 - validItems) + " incomplete column(s).");
      for (int i = 0; i < 4; i++) {
        BcSmartspaceTemplateDataUtils.updateVisibility(
            getChildAt(i),
            i <= (3 - (4 - validItems)) ? View.VISIBLE : View.GONE);
      }
    ...
    for (int i = 0; i < validItems; i++) {
      ...
      BcSmartspaceTemplateDataUtils.setText(
          upperText, ((CarouselTemplateData.CarouselItem)carouselItems.get(i))
                         .getUpperText());
      BcSmartspaceTemplateDataUtils.updateVisibility(upperText, View.VISIBLE);
      BcSmartspaceTemplateDataUtils.setIcon(
          icon,
          ((CarouselTemplateData.CarouselItem)carouselItems.get(i)).getImage());
      BcSmartspaceTemplateDataUtils.updateVisibility(icon, View.VISIBLE);
      BcSmartspaceTemplateDataUtils.setText(
          lowerText, ((CarouselTemplateData.CarouselItem)carouselItems.get(i))
                         .getLowerText());
      BcSmartspaceTemplateDataUtils.updateVisibility(lowerText, View.VISIBLE);
    }
    if (templateData.getCarouselAction() != null) {
      BcSmartSpaceUtil.setOnClickListener(
          this, target, templateData.getCarouselAction(), eventNotifier,
          "CarouselTemplateCard", loggingInfo, 0);
    }
    for (CarouselTemplateData.CarouselItem item :
         templateData.getCarouselItems()) {
