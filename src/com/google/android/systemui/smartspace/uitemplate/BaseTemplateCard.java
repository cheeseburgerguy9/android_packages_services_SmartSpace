...
public class BaseTemplateCard
    extends ConstraintLayout implements SmartspaceCard {
  ...
  public BaseTemplateCard(Context context) { this(context, null); }
  ...
  @Override
  public final void
  bindData(SmartspaceTarget target,
    ...
    BcSmartspaceTemplateDataUtils.updateVisibility(mSubtitleTextView,
                                                   View.GONE);
    BcSmartspaceTemplateDataUtils.updateVisibility(mSubtitleSupplementalView,
                                                   View.GONE);
    ...
    if (mSecondaryCard != null) {
      ...
      mValidSecondaryCard = mSecondaryCard.setSmartspaceActions(
          target, eventNotifier, mLoggingInfo);
    }
    if (mSecondaryCardPane != null) {
      BcSmartspaceTemplateDataUtils.updateVisibility(
          mSecondaryCardPane, (mDozeAmount == 1.0f || !mValidSecondaryCard)
                                  ? View.GONE
                                  : View.VISIBLE);
    }
    if (mDateView == null) {
      ...
      TapAction tapAction =
          new TapAction
              ...
              .build();
      BcSmartSpaceUtil.setOnClickListener(this, mTarget, tapAction,
                                          eventNotifier, "SsBaseTemplateCard",
                                          loggingInfo, 0);
      BcSmartSpaceUtil.setOnClickListener(mDateView, mTarget, tapAction,
                                          eventNotifier, "SsBaseTemplateCard",
                                          loggingInfo, 0);
    }
    setUpTextView(mTitleTextView, mTemplateData.getPrimaryItem(), eventNotifier,
                  mDateView == null);
    setUpTextView(mSubtitleTextView, mTemplateData.getSubtitleItem(),
                  eventNotifier, true);
    setUpTextView(mSubtitleSupplementalView,
                  mTemplateData.getSubtitleSupplementalItem(), eventNotifier,
                  true);
    setUpTextView(mSupplementalLineTextView,
                  mTemplateData.getSupplementalLineItem(), eventNotifier, true);
    if (mExtrasGroup != null) {
      if (mSupplementalLineTextView == null ||
        ...
        BcSmartspaceTemplateDataUtils.updateVisibility(mExtrasGroup,
                                                       View.VISIBLE);
        updateZenColors();
      }
    }
    ...
    if (mDateView == null && mTemplateData.getPrimaryItem() != null &&
      ...
      BcSmartSpaceUtil.setOnClickListener(
          this, target, mTemplateData.getPrimaryItem().getTapAction(),
          eventNotifier, "SsBaseTemplateCard", mLoggingInfo, 0);
      if (mDateView == null && mTitleTextView != null &&
        ...
        if (mTemplateData.getPrimaryItem().getTapAction().getIntent() != null &&
            !mTemplateData.getPrimaryItem()
                 .getTapAction()
                 .getIntent()
                 .filterEquals(mTemplateData.getSubtitleItem()
                                   .getTapAction()
                                   .getIntent())) {
        ...
        } else if (mTemplateData.getPrimaryItem()
                           .getTapAction()
                           .getPendingIntent() == null ||
                   mTemplateData.getPrimaryItem()
                       .getTapAction()
                       .getPendingIntent()
                       .equals(mTemplateData.getSubtitleItem()
                                   .getTapAction()
                                   .getPendingIntent())) {
  ...
  public final void onFinishInflate() {
    ...
    if (mExtrasGroup != null) {
      mSupplementalLineTextView =
          mExtrasGroup.findViewById(R.id.supplemental_line_text);
    }
    if (mBgHandler != null) {
  ...
  protected void onLayout(boolean changed, int left, int top, int right,
    ...
    if (subtitleTextVisible) {
      ...
      composite.mDelegates.add(
          new TouchDelegate(mSubtitleHitRect, mSubtitleTextView));
    }
    if (subtitleSupplementalVisible) {
      mSubtitleSupplementalView.getHitRect(mSubtitleSupplementalHitRect);
      offsetDescendantRectToMyCoords(mSubtitleGroup,
                                     mSubtitleSupplementalHitRect);
      if (padding > 0) {
        mSubtitleSupplementalHitRect.top -= padding;
      }
      mSubtitleSupplementalHitRect.bottom = getBottom();
      composite.mDelegates.add(new TouchDelegate(mSubtitleSupplementalHitRect,
  ...
  public final void setDozeAmount(float dozeAmount) {
    ...
    if (mTarget != null && mTarget.getBaseAction() != null &&
      ...
      if (mSubtitleTextView != null &&
          extras.getBoolean("hide_subtitle_on_aod")) {
    ...
    BcSmartspaceTemplateDataUtils.updateVisibility(
        mSecondaryCardPane, (mDozeAmount == 1.0f || !mValidSecondaryCard)
                                ...
                                : View.VISIBLE);
    if (mSecondaryCardPane == null ||
        mSecondaryCardPane.getVisibility() == View.GONE) {
      mTextGroup.setTranslationX(0.0f);
      return;
    }
    mTextGroup.setTranslationX(((PathInterpolator)Interpolators.EMPHASIZED)
                                   .getInterpolation(mDozeAmount) *
  ...
  public final void setPrimaryTextColor(int color) {
    ...
    if (mTitleTextView != null) {
      ...
      if (mTemplateData != null) {
        updateTextViewIconTint(mTitleTextView,
                               shouldTint(mTemplateData.getPrimaryItem()));
      }
    }
    if (mDateView != null) {
    ...
    if (mSubtitleTextView != null) {
      ...
      if (mTemplateData != null) {
        updateTextViewIconTint(mSubtitleTextView,
                               shouldTint(mTemplateData.getSubtitleItem()));
      }
    }
    if (mSubtitleSupplementalView != null) {
  ...
  public final void setUpTextView(
    ...
    if (mLoggingInfo != null && mLoggingInfo.mSubcardInfo != null &&
      ...
      if (targetFeatureType != mLoggingInfo.mFeatureType) {
        for (int i = 0; i < mLoggingInfo.mSubcardInfo.mSubcards.size(); i++) {
          ...
          if (subCard.mInstanceId ==
                  subItemInfo.getLoggingInfo().getInstanceId() &&
    ...
    BcSmartSpaceUtil.setOnClickListener(textView, mTarget, tapAction,
                                        eventNotifier, "SsBaseTemplateCard",
                                        mLoggingInfo, subCardRank);
  }

  public final void updateTextViewIconTint(DoubleShadowTextView textView,
                                           boolean shouldTint) {
  ...
  public BaseTemplateCard(Context context, AttributeSet attrs) {
    ...
    mLoggingInfo = null;
    mIconTintColor =
        GraphicsUtils.getAttrColor(context, android.R.attr.textColorPrimary);
    mTextGroup = null;
