...
public class SubListTemplateCard extends BcSmartspaceCardSecondary {
  ...
  public final boolean setSmartspaceActions(
    ...
    SubListTemplateData templateData =
        (SubListTemplateData)target.getTemplateData();
    ...
    if (templateData.getSubListIcon() != null) {
      BcSmartspaceTemplateDataUtils.setIcon(mListIconView,
                                            templateData.getSubListIcon());
      BcSmartspaceTemplateDataUtils.updateVisibility(mListIconView,
                                                     View.VISIBLE);
    } else {
      BcSmartspaceTemplateDataUtils.updateVisibility(mListIconView, View.GONE);
    }
    if (templateData.getSubListTexts() != null) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (i < subListTexts.size()) {
          ...
          BcSmartspaceTemplateDataUtils.updateVisibility(textView,
                                                         View.VISIBLE);
        } else {
          textView.setText("");
    ...
    if (templateData.getSubListAction() != null) {
      BcSmartSpaceUtil.setOnClickListener(
          this, target, templateData.getSubListAction(), eventNotifier,
          "SubListTemplateCard", loggingInfo, 0);
    }
    return true;
  }
