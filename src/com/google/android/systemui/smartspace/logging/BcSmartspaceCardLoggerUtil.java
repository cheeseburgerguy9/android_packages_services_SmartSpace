...
public abstract class BcSmartspaceCardLoggerUtil {
  public static boolean containsValidTemplateType(BaseTemplateData data) {
    return (data == null || data.getTemplateType() == 0 ||
            data.getTemplateType() == 8)
        ? false
        : true;
  }

  public static SmartspaceProto.SmartspaceCardDimensionalInfo
  createDimensionalLoggingInfo(BaseTemplateData data) {
    ...
    List<SmartspaceProto.SmartspaceFeatureDimension> dimensions =
        new ArrayList<>();

    if (extras != null && !extras.isEmpty()) {
      ArrayList<Integer> ids =
          extras.getIntegerArrayList("ss_card_dimension_ids");
      ArrayList<Integer> values =
          extras.getIntegerArrayList("ss_card_dimension_values");
  ...
  public static BcSmartspaceSubcardLoggingInfo
  createSubcardLoggingInfo(SmartspaceTarget target) {
    ...
    int instanceId = InstanceId.create(
        target.getBaseAction().getExtras().getString("subcardId"));
    ...
    BcSmartspaceSubcardLoggingInfo subcardInfo =
        new BcSmartspaceSubcardLoggingInfo();
  ...
  public static BcSmartspaceSubcardLoggingInfo
  createSubcardLoggingInfo(BaseTemplateData data) {
    if (data == null) {
      return null;
    }
    ...
    createSubcardLoggingInfoHelper(subcards,
                                   data.getSubtitleSupplementalItem());
