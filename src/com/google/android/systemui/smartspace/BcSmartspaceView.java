...
public class BcSmartspaceView extends FrameLayout
      ...
      if (state == ViewPager2.SCROLL_STATE_IDLE) {
        if (mConfigProvider.isSwipeEventLoggingEnabled() &&
            ...
            (cardAtPosition = mAdapter.getCardAtPosition(
                 mSwipedCardPosition.intValue())) != null) {
  ...
  public final class ViewPagerOnPageChangeListener
  ...
  public static void onViewPagerPageSelected(BcSmartspaceView view,
                                             int position) {
    SmartspaceTarget previousTarget =
        view.mAdapter.getTargetAtPosition(view.mCardPosition);
    view.mCardPosition = position;
    SmartspaceTarget currentTarget =
        view.mAdapter.getTargetAtPosition(position);
    if (currentTarget != null) {
      view.logSmartspaceEvent(currentTarget, view.mCardPosition,
                              BcSmartspaceEvent.SMARTSPACE_CARD_SEEN);
    }
    if (view.mDataProvider == null) {
      Log.w("BcSmartspaceView", "Cannot notify target hidden/shown " +
                                "smartspace events: data provider null");
      return;
    }
    if (previousTarget == null) {
      Log.w("BcSmartspaceView", "Cannot notify target hidden smartspace " +
                                "event: previous target is null.");
    } else {
      ...
      view.mDataProvider.getEventNotifier().notifySmartspaceEvent(
          builder.build());
    }
    if (currentTarget == null) {
      Log.w("BcSmartspaceView", "Cannot notify target shown smartspace " +
                                "event: shown card smartspace target null.");
      return;
    }
    SmartspaceTargetEvent.Builder builder =
    ...
    mAodObserver = new ContentObserver(new Handler()) {
      ...
      public final void onChange(boolean selfChange) {
        mIsAodEnabled = Settings.Secure.getIntForUser(
                            getContext().getContentResolver(), "doze_always_on",
                            0, getContext().getUserId()) == 1;
      }
    };
  ...
  public int getCurrentCardTopPadding() {
    ...
    BaseTemplateCard templateCard =
        mAdapter.getTemplateCardAtPosition(position);
  ...
  handleTouchOverride(MotionEvent event, Predicate<MotionEvent> touchHandler) {
    ...
    if (mViewPager2 != null) {
      ...
      if (action == 0) {
        ...
        if (mViewPager2.isLongClickable()) {
          ...
          mViewPager2.postDelayed(mLongPressCallback,
  ...
  public final void logSmartspaceEvent(SmartspaceTarget target, int rank,
                                       BcSmartspaceEvent event) {
    ...
    if (event == BcSmartspaceEvent.SMARTSPACE_CARD_RECEIVED) {
      try {
        ...
        Log.e("BcSmartspaceView",
              "received_latency_millis will be -1 due to exception ", e);
          ...
          try {
            if (mBgHandler == null) {
              throw new IllegalStateException(
                  "Must set background handler to avoid making binder calls " +
                  "on main thread");
            }
            mBgHandler.post(() -> {
              ...
              mIsAodEnabled = Settings.Secure.getIntForUser(
                                  resolver, "doze_always_on", 0, userId) == 1;
    ...
    if (mViewPager2 != null) {
      if (mAdapter instanceof CardRecyclerViewAdapter) {
        ...
        mViewPager2.registerOnPageChangeCallback(
            mViewPager2OnPageChangeCallback);
        if (mPagerDots != null) {
        }
        if (TextUtils.equals(
                mAdapter.getUiSurface(),
  ...
  public final void onFinishInflate() {
    ...
    if (pager instanceof ViewPager) {
      ...
      if (!(pager instanceof ViewPager2)) {
        throw new IllegalStateException(
            "smartspace_card_pager is an invalid view type");
      }
  ...
  @Override
  public final void
  onSmartspaceTargetsUpdated(List<? extends Parcelable> targets) {
    ...
    View templateCardAtPosition =
        mAdapter.getTemplateCardAtPosition(selectedPage);
    BcSmartspaceCard legacyCardAtPosition =
        mAdapter.getLegacyCardAtPosition(selectedPage);
    ...
    if (templateCardAtPosition == null) {
      templateCardAtPosition = legacyCardAtPosition != null
                                   ? legacyCardAtPosition
                                   : remoteViewsCardAtPosition;
    }
  ...
  @Override
  public final void
  registerConfigProvider(BcSmartspaceConfigPlugin configProvider) {
  ...
  public final void setDozeAmount(float dozeAmount) {
    ...
    if (mPagerDots != null) {
      ...
      if (mPagerDots.getVisibility() != View.GONE) {
        if (dozeAmount == 1.0f) {
          BcSmartspaceTemplateDataUtils.updateVisibility(mPagerDots,
                                                         View.INVISIBLE);
        } else {
          BcSmartspaceTemplateDataUtils.updateVisibility(mPagerDots,
                                                         View.VISIBLE);
    ...
    if (displaySurface != 3 || mIsAodEnabled) {
      ...
      if (target == null) {
        ...
        logSmartspaceEvent(target, mCardPosition,
                           BcSmartspaceEvent.SMARTSPACE_CARD_SEEN);
      }
    }
  }
  ...
  public final void setHorizontalPaddings(int padding) {
    if (mPagerDots != null) {
      mPagerDots.setPaddingRelative(padding, mPagerDots.getPaddingTop(),
                                    padding, mPagerDots.getPaddingBottom());
    }
  ...
  public final void setTargets(boolean z, int i, View view, int i2) {
    ...
    for (int i3 = 0; i3 < count; i3++) {
      ...
      if (!mLastReceivedTargets.contains(
        ...
        logSmartspaceEvent(targetAtPosition, i3,
    ...
    mLastReceivedTargets.addAll(
        (Collection)mAdapter.getSmartspaceTargets()
            .stream()
