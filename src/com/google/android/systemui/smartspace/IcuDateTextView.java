...
public class IcuDateTextView extends DoubleShadowTextView {
  ...
  public IcuDateTextView(Context context) { this(context, null); }
  ...
  public final void onAttachedToWindow() {
    ...
    if (mUpdatesOnAod) {
      try {
        if (mBgHandler == null) {
          Log.wtf(TAG, "Must set background handler when mUpdatesOnAod is " +
                       "set to avoid making binder calls on main thread");
    ...
    if (mBgHandler == null) {
      Log.w(TAG, "mBgHandler is not set! Fallback to make binder calls on " +
                 "main thread.");
  ...
  public final void onDetachedFromWindow() {
    ...
    if (mUpdatesOnAod) {
      if (mBgHandler == null) {
        Log.wtf(TAG, "Must set background handler when mUpdatesOnAod is set " +
                     "to avoid making binder calls on main thread");
  ...
  public final void onTimeChanged(boolean forceUpdateFormatter) {
    if (mFormatter == null || forceUpdateFormatter) {
      mFormatter = DateFormat.getInstanceForSkeleton(
          ...
          Locale.getDefault());
      mFormatter.setContext(
          DisplayContext.CAPITALIZATION_FOR_BEGINNING_OF_SENTENCE);
    }
    String newText =
        mFormatter.format(Long.valueOf(System.currentTimeMillis()));
  ...
  public final void rescheduleTicker() {
    ...
    mTimeChangedDelegate.unregister();
    if ((mIsInteractive == null || mIsInteractive ||
         (mUpdatesOnAod && mIsAodEnabled)) &&
        isAggregatedVisible()) {
    ...
    mIntentReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        boolean updateFormatter =
            "android.intent.action.TIMEZONE_CHANGED".equals(
                intent.getAction()) ||
