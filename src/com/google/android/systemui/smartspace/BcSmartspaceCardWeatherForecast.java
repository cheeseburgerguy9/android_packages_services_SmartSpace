...
public class BcSmartspaceCardWeatherForecast extends BcSmartspaceCardSecondary {
  ...
  public BcSmartspaceCardWeatherForecast(Context context) { super(context); }
  ...
  protected void onFinishInflate() {
    ...
    while (i2 < 4) {
      Constraints.LayoutParams layoutParams =
          new Constraints.LayoutParams(-2, 0);
      ConstraintLayout constraintLayout2 = constraintLayoutArr[i2];
      ConstraintLayout constraintLayout3 =
          i2 > 0 ? constraintLayoutArr[i2 - 1] : null;
      ConstraintLayout constraintLayout4 =
          i2 < 3 ? constraintLayoutArr[i2 + 1] : null;
      if (i2 == 0) {
    ...
    if (extras.containsKey("temperatureValues")) {
      ...
      if (stringArray == null) {
        Log.w("BcSmartspaceCardWeatherForecast",
              "Temperature values array is null.");
      } else {
        updateFields((view, i)
                         -> ((TextView)view).setText(stringArray[i]),
                     stringArray.length, R.id.temperature_value,
                     "temperature value");
      }
      z = true;
    } else {
  ...
  public final void updateFields(ItemUpdateFunction itemUpdateFunction,
                                 int count, int viewId, String viewName) {
    if (getChildCount() < 4) {
      Log.w("BcSmartspaceCardWeatherForecast",
    ...
    if (count < 4) {
      ...
      Log.w("BcSmartspaceCardWeatherForecast",
            String.format(Locale.US,
                          "Missing %d %s(s). Hiding incomplete columns.", i3,
                          viewName));
      if (getChildCount() < 4) {
        Log.w("BcSmartspaceCardWeatherForecast",
        ...
        int i4 = 3 - i3;
        for (int i = 0; i < 4; i++) {
          BcSmartspaceTemplateDataUtils.updateVisibility(getChildAt(i),
                                                         i <= i4 ? 0 : 8);
        }
